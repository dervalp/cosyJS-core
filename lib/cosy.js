...
Cosy.prototype.getScript = function( name ) {
    var result = [],
        compMetaData = this._c.scripts[ name ];

    if(compMetaData.adapter) {
        result = result.concat(this.getAdapter(compMetaData.adapter));
    }

    if(compMetaData.plugins) {
        result = result.concat(this.getPlugin(compMetaData.plugins));
    }

    if(compMetaData.path) {
        result = result.concat(compMetaData.path);
    }

    return result;
};

Cosy.prototype.getAdapter = function( name ) {
    return this._c.adapters[ name ].path;
};

Cosy.prototype.getPlugin = function( name ) {
    return this._c.plugins[ name ].path;
};
